...
public class ValidationViews {
    ...
    static String panpattern3 = "[A-Z]{5}[0-9]{4}[A-Z]{1}";
    static String passportRegex = "[A-Z]{1}[0-9]{7}";
    ...
    public static boolean EditTextPanPatternValidate(EditText... testObj) {
        for (int i = 0; i < testObj.length; i++) {
            if (!testObj[i].getText().toString().trim().matches(panpattern3)) {
                testObj[i].setError("Invalid Pan");
                testObj[i].requestFocus();
    ...
    public static boolean EditTextPassportPatternValidate(EditText... testObj) {
        for (int i = 0; i < testObj.length; i++) {
            if (!testObj[i].getText().toString().trim().matches(passportRegex)) {
                testObj[i].setError("Invalid Pan");
                testObj[i].requestFocus();
